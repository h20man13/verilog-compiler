...
RegDeclaration::RegDeclaration(const Position& position, std::list<RegValue* const> &reg_list): Declaration(position){
	for(std::list<RegValue* const>::const_iterator it = reg_list.begin(); it != reg_list.end(); it++){
		this->reg_list.push_back(*it);
	}
}

RegDeclaration::RegDeclaration(const RegDeclaration& declaration): Declaration(declaration.get_position()){
